  ...
  useEffect(() => {
    const subscriber = firestore()
      ...
      .onSnapshot(async querySnapshot => {
        ...
        const customerID = user.key; // Replace with the desired customerID
        const filteredTransactions = allTransactions.filter(transaction => transaction.customerID === customerID);
  ...
  const renderItem = ({item}) => {
    ...
    return (
      <TouchableOpacity
        ...
        onPress={() =>
          navigation.navigate('TransactionDetail', {transaction: item, user})
        }>
